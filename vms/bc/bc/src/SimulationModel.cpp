...
SimulationModel::SimulationModel(
    irr::IrrlichtDevice* dev, irr::scene::ISceneManager* scene, GUIMain* gui,
    Sound* sound, ScenarioData scenarioData, OperatingMode::Mode mode,
    irr::f32 viewAngle, irr::f32 lookAngle, irr::f32 cameraMinDistance,
    irr::f32 cameraMaxDistance, irr::u32 disableShaders, irr::u32 waterSegments,
    irr::core::vector3di numberOfContactPoints, irr::u32 limitTerrainResolution)
    : manOverboard(irr::core::vector3df(0, 0, 0), scene, dev, this,
                   &terrain)  // Initialise MOB
{
  ...
  // Store a serialised form of the scenario loaded, as we may want to send this
  // over the network
  ...
  if (visibilityRange <= 0) {
    visibilityRange = 5 * M_IN_NM;
  }  // TODO: Check units

  // Fixme: Think about time zone handling
  // Fixme: Note that if the time_t isn't long enough, 2038 problem exists
  scenarioOffsetTime = Utilities::dmyToTimestamp(
      startDay, startMonth,
      startYear);  // Time in seconds to start of scenario day (unix timestamp
                   // for 0000h on day scenario starts)
  ...
  // Load own ship model.
  ownShip.load(scenarioData.ownShipData, numberOfContactPoints, smgr, this,
               &terrain, device);
  ...
  // add water
  water.load(smgr, ownShip.getSceneNode(), weather, disableShaders,
             waterSegments);
...
//	DEE_NOV22 todo hide schottels engine indicators etc
  ...
  */
  ...
  std::vector<irr::core::vector3df> views =
      ownShip.getCameraViews();  // Get the initial camera offset from the own
                                 // ship model
  std::vector<bool> isHighView =
      ownShip.getCameraIsHighView();  // Are these special 'looking down' views
  irr::f32 angleCorrection = ownShip.getAngleCorrection();
  camera.load(smgr, device->getLogger(), ownShip.getSceneNode(), views,
              isHighView, irr::core::degToRad(viewAngle), lookAngle,
              angleCorrection);
  ...
  radarScreen.load(
      smgr, ownShip.getSceneNode(), ownShip.getScreenDisplayPosition(),
      ownShip.getScreenDisplaySize(), ownShip.getScreenDisplayTilt());

  // make radar image - one for the background render, and one with any 2d
  // drawing on top Make as big as the maximum screen display size (next power
  // of 2), and then only use as much as is needed to get 1:1 image to screen
  // pixel mapping
  irr::u32 radarTextureSize =
      driver->getScreenSize().Height *
      0.4;  // Optimised for the small radar screen (Where 0.6*screen height is
            // used for the 3d view). We should have a higher resolution for
            // full radar view
  irr::u32 largeRadarTextureSize =
      driver->getScreenSize().Height;  // Optimised for the large radar screen
  // Find next power of 2 size
  radarTextureSize = std::pow(2, std::ceil(std::log2(radarTextureSize)));
  largeRadarTextureSize =
      std::pow(2, std::ceil(std::log2(largeRadarTextureSize)));

  // In simulationModel, keep track of the used size, and pass this to gui etc.
  radarImage = driver->createImage(
      irr::video::ECF_A8R8G8B8,
      irr::core::dimension2d<irr::u32>(
          radarTextureSize,
          radarTextureSize));  // Create image for radar calculation to work on
  radarImageOverlaid = driver->createImage(
      irr::video::ECF_A8R8G8B8,
      irr::core::dimension2d<irr::u32>(
          radarTextureSize,
          radarTextureSize));  // Create image for radar calculation to work on
  radarImageLarge = driver->createImage(
      irr::video::ECF_A8R8G8B8,
      irr::core::dimension2d<irr::u32>(
          largeRadarTextureSize,
          largeRadarTextureSize));  // Create image for radar calculation to
                                    // work on
  radarImageOverlaidLarge = driver->createImage(
      irr::video::ECF_A8R8G8B8,
      irr::core::dimension2d<irr::u32>(
          largeRadarTextureSize,
          largeRadarTextureSize));  // Create image for radar calculation to
  ...
  // fill with bg colour
  radarImage->fill(
      irr::video::SColor(255, 128, 128, 128));  // Fill with background colour
  radarImageOverlaid->fill(
      irr::video::SColor(255, 128, 128, 128));  // Fill with background colour
  radarImageLarge->fill(
      irr::video::SColor(255, 128, 128, 128));  // Fill with background colour
  radarImageOverlaidLarge->fill(
      irr::video::SColor(255, 128, 128, 128));  // Fill with background colour

  // make radar camera
  std::vector<irr::core::vector3df>
      radarViews;  // Get the initial camera offset from the radar screen
  std::vector<bool> radarViewsLookDown;  // Not needed for the radar camera, but
                                         // needed for compatability
  irr::f32 screenTilt = ownShip.getScreenDisplayTilt();
  radarViews.push_back(
      ownShip.getScreenDisplayPosition() +
      irr::core::vector3df(0,
                           0.5 * sin(irr::core::DEGTORAD * screenTilt) *
                               ownShip.getScreenDisplaySize(),
                           -0.5 * cos(irr::core::DEGTORAD * screenTilt) *
                               ownShip.getScreenDisplaySize()));
  radarViewsLookDown.push_back(false);
  radarCamera.load(smgr, device->getLogger(), ownShip.getSceneNode(),
                   radarViews, radarViewsLookDown, irr::core::PI / 2.0, 0, 0);
  radarCamera.setLookUp(
      -1.0 * screenTilt);  // FIXME: Why doesn't simply -1.0*screenTilt work?
...
SimulationModel::~SimulationModel() {
  radarImage->drop();  // We created this with 'create', so drop it when we're
                       // finished
  radarImageOverlaid->drop();  // We created this with 'create', so drop it when
                               // we're finished
  radarImageLarge->drop();     // We created this with 'create', so drop it when
                               // we're finished
  radarImageOverlaidLarge->drop();  // We created this with 'create', so drop it
                                    // when we're finished
...
void SimulationModel::setSpeed(irr::f32 spd) { ownShip.setSpeed(spd); }

irr::f32 SimulationModel::getSpeed() const { return (ownShip.getSpeed()); }
...
irr::f32 SimulationModel::gnss_noise() const {
  // lat and lon have a 95% probability of being offset by at most
  // 6 meters (two standard deviations).
  static std::random_device random_device;
  static std::mt19937 gnss_rng(random_device());
  static std::normal_distribution<double> gnss_d(0, 3);
  irr::f32 offset = gnss_d(gnss_rng);
  // clamp between +- 40 meters of error
  return std::min(std::max(offset, -40.0f), 40.0f);
}

irr::f32 SimulationModel::getNoisyLat() const {
  return terrain.zToLat(ownShip.getPosition().Z + offsetPosition.Z +
                        gnss_noise());
}

irr::f32 SimulationModel::getNoisyLong() const {
  return terrain.xToLong(ownShip.getPosition().X + offsetPosition.X +
                         gnss_noise());
}
...
irr::f32 SimulationModel::getCOG() const { return ownShip.getCOG(); }

irr::f32 SimulationModel::getSOG() const { return ownShip.getSOG(); }

irr::f32 SimulationModel::getDepth() const { return ownShip.getDepth(); }

irr::f32 SimulationModel::getShipDepth() const {
  return ownShip.getShipDepth();
}

irr::f32 SimulationModel::getBuoyancy() const { return ownShip.getBuoyancy(); }

irr::f32 SimulationModel::getWaveHeight(irr::f32 posX, irr::f32 posZ) const {
  return water.getWaveHeight(posX, posZ);
}

irr::core::vector2df SimulationModel::getLocalNormals(irr::f32 relPosX,
                                                      irr::f32 relPosZ) const {
  return water.getLocalNormals(relPosX, relPosZ);
}

irr::core::vector2df SimulationModel::getTidalStream(
    irr::f32 longitude, irr::f32 latitude, uint64_t absoluteTime) const {
  return tide.getTidalStream(longitude, latitude, absoluteTime);
}

// void SimulationModel::getTime(irr::u8& hour, irr::u8& min, irr::u8& sec)
// const{
//    //FIXME: Complete
// }

// void SimulationModel::getDate(irr::u8& day, irr::u8& month, irr::u16& year)
// const{
//     //FIXME: Complete
// }

uint64_t SimulationModel::getTimestamp() const { return absoluteTime; }

uint64_t SimulationModel::getTimeOffset()
    const {  // The timestamp at the start of the first day of the scenario
...
irr::f32 SimulationModel::getTimeDelta()
    const {  // The change in time (s) since the start of the start day of the
             // scenario
  return scenarioTime;
}
...
irr::u32 SimulationModel::getNumberOfBuoys() const { return buoys.getNumber(); }
...
void SimulationModel::setOtherShipPos(int number, irr::f32 positionX,
                                      irr::f32 positionZ) {
  otherShips.setPos(number, positionX - offsetPosition.X,
                    positionZ - offsetPosition.Z);
}
...
void SimulationModel::changeOtherShipLeg(int shipNumber, int legNumber,
                                         irr::f32 bearing, irr::f32 speed,
                                         irr::f32 distance) {
  otherShips.changeLeg(shipNumber, legNumber, bearing, speed, distance,
                       scenarioTime);
}

void SimulationModel::addOtherShipLeg(int shipNumber, int afterLegNumber,
                                      irr::f32 bearing, irr::f32 speed,
                                      irr::f32 distance) {
  otherShips.addLeg(shipNumber, afterLegNumber, bearing, speed, distance,
                    scenarioTime);
}
...
void SimulationModel::resetOtherShipLegs(int shipNumber, irr::f32 course,
                                         irr::f32 speedKts,
                                         irr::f32 distanceNm) {
...
std::string SimulationModel::getOwnShipEngineSound() const {
  // Check existence of sound file in base path, and if not fall back to
  // default.
...
void SimulationModel::setHeading(irr::f32 hdg) { ownShip.setHeading(hdg); }
...
void SimulationModel::setPos(irr::f32 positionX, irr::f32 positionZ) {
  ownShip.setPosition(positionX - offsetPosition.X,
                      positionZ - offsetPosition.Z);
}

irr::f32 SimulationModel::getHeading() const { return (ownShip.getHeading()); }
...
void SimulationModel::setBallastTankPump(irr::f32 pump) {
  ownShip.setBallastTankPump(pump);
}

irr::f32 SimulationModel::getRudder() const { return ownShip.getRudder(); }
...
irr::f32 SimulationModel::getWheel() const { return ownShip.getWheel(); }
// DEE ^^^^^^^^^^^

void SimulationModel::setAzimuth1Master(
    bool isMaster) {  // Set if azimuth 1 should also control azimuth 2
  ownShip.setAzimuth1Master(isMaster);
}

void SimulationModel::setAzimuth2Master(
    bool isMaster) {  // Set if azimuth 2 should also control azimuth 1
...
void SimulationModel::setPortSchottel(
    irr::f32 portAngle) {  // Set the Port Schottel control angle in degrees
                           // (-ve is anticlockwise, +ve is clockwise)
  ownShip.setPortSchottel(portAngle);
}

void SimulationModel::setStbdSchottel(
    irr::f32 stbdAngle) {  // Set the Stbd Schottel control angle in degrees
                           // (-ve is anticlockwise, +ve is clockwise)
  ownShip.setStbdSchottel(stbdAngle);
}

irr::f32
SimulationModel::getPortSchottel() {  // Gets the Port Schottel angle, (-ve is
                                      // anticlockwise, +ve is clockwise)
  return ownShip.getPortSchottel();
}

irr::f32
SimulationModel::getStbdSchottel() {  // Gets the Stbd Schottel angle, (-ve is
                                      // anticlockwise, +ve is clockwise)
  return ownShip.getStbdSchottel();
}

// DEE_NOV22 btn control of shcottels ... this is for when you dont use a mouse
// of control console,
//           however it is also close enough to emergency steering mode of
//           azimuth drives for all
...
void SimulationModel::btnIncrementStbdSchottel() {
  ownShip.btnIncrementStbdSchottel();  // DEE_NOV22 stbd shcottel clockwise in
                                       // response to KEY_KEY_L
}

void SimulationModel::btnDecrementStbdSchottel() {
  ownShip.btnDecrementStbdSchottel();  // DEE_NOV22 port schottel anticlockwise
                                       // in response to KEY_KEY_J
}
...
// DEE_NOV22 Clutches , in normal operation these would be automatic, however in
// emergency (non follow up) mode they are manual DEE_NOV22 in future perhaps
// model engine stall for when clutch engaged at too low a revs and prop shaft
// snap if clutch DEE_NOV22 is engaged at too high a revs
...
bool SimulationModel::getPortClutch() { return ownShip.getPortClutch(); }

bool SimulationModel::getStbdClutch() { return ownShip.getStbdClutch(); }

// DEE_NOV22 todo need to assign keys to this is emergency steering mode where
// there is no automatic clutch
//           I think we could use the follow up / non follow up flag to
//           determine if it is in normal or
//		 emergency steering mode.
//		 todo is it better to use SimulationModel::setXXXXClutch(xxxx)
// for this

void SimulationModel::engagePortClutch() { ownShip.setPortClutch(true); }

void SimulationModel::disengagePortClutch() { ownShip.setPortClutch(false); }

void SimulationModel::engageStbdClutch() { ownShip.setStbdClutch(true); }

void SimulationModel::disengageStbdClutch() { ownShip.setStbdClutch(false); }

// DEE_NOV22 ^^^^ Azimuth Drive follow up code

void SimulationModel::setPortAzimuthAngle(
    irr::f32 angle) {  // Set the azimuth angle, in degrees (-ve is port, +ve is
                       // stbd)
  ownShip.setPortAzimuthAngle(angle);
}

void SimulationModel::setStbdAzimuthAngle(
    irr::f32 angle) {  // Set the azimuth angle, in degrees (-ve is port, +ve is
                       // stbd)
...
void SimulationModel::setPortEngine(irr::f32 port) {
  ...
  if (ownShip.isSingleEngine()) {
    ...
    sound->setVolumeEngine((fabs(getPortEngine()) + fabs(getStbdEngine())) *
                           0.5);
  }
}
...
void SimulationModel::setWeather(irr::f32 weather) { this->weather = weather; }

irr::f32 SimulationModel::getWeather() const { return weather; }

void SimulationModel::setRain(irr::f32 rainIntensity) {
  this->rainIntensity = rainIntensity;
}

irr::f32 SimulationModel::getRain() const { return rainIntensity; }

void SimulationModel::setVisibility(irr::f32 visibilityNm) {
  this->visibilityRange = visibilityNm;
}

irr::f32 SimulationModel::getVisibility() const { return visibilityRange; }

void SimulationModel::setWaterVisible(bool visible) {
  water.setVisible(visible);
}

void SimulationModel::set_water_transparent(bool trans) {
  water.set_transparent(trans);
}

void SimulationModel::lookUp() { camera.lookUp(); }

void SimulationModel::lookDown() { camera.lookDown(); }

void SimulationModel::lookLeft() { camera.lookLeft(); }

void SimulationModel::lookRight() { camera.lookRight(); }
...
void SimulationModel::lookStepLeft() { camera.lookStepLeft(); }

void SimulationModel::lookStepRight() { camera.lookStepRight(); }

void SimulationModel::moveCameraForwards() { camera.moveForwards(); }

void SimulationModel::moveCameraBackwards() { camera.moveBackwards(); }

void SimulationModel::lookAhead() { camera.lookAhead(); }

void SimulationModel::lookAstern() { camera.lookAstern(); }

void SimulationModel::lookPort() { camera.lookPort(); }

void SimulationModel::lookStbd() { camera.lookStbd(); }
...
irr::u32 SimulationModel::getCameraView() const { return camera.getView(); }
...
void SimulationModel::toggleRadarOn() { radarCalculation.toggleRadarOn(); }

bool SimulationModel::isRadarOn() const { return radarCalculation.isRadarOn(); }

void SimulationModel::increaseRadarRange() { radarCalculation.increaseRange(); }

void SimulationModel::decreaseRadarRange() { radarCalculation.decreaseRange(); }
...
void SimulationModel::setPIData(irr::s32 PIid, irr::f32 PIbearing,
                                irr::f32 PIrange) {
...
void SimulationModel::increaseRadarEBLRange() {
  radarCalculation.increaseEBLRange();
}
void SimulationModel::decreaseRadarEBLRange() {
  radarCalculation.decreaseEBLRange();
}
void SimulationModel::increaseRadarEBLBrg() {
  radarCalculation.increaseEBLBrg();
}
void SimulationModel::decreaseRadarEBLBrg() {
  radarCalculation.decreaseEBLBrg();
}

void SimulationModel::setRadarNorthUp() { radarCalculation.setNorthUp(); }

void SimulationModel::setRadarCourseUp() { radarCalculation.setCourseUp(); }

void SimulationModel::setRadarHeadUp() { radarCalculation.setHeadUp(); }

void SimulationModel::changeRadarColourChoice() {
  radarCalculation.changeRadarColourChoice();
}

void SimulationModel::setArpaOn(bool on) { radarCalculation.setArpaOn(on); }

void SimulationModel::setRadarARPARel() { radarCalculation.setRadarARPARel(); }
...
void SimulationModel::setMainCameraActive() { camera.setActive(); }

void SimulationModel::setRadarCameraActive() { radarCamera.setActive(); }
...
irr::u32 SimulationModel::getLoopNumber() const { return loopNumber; }

std::string SimulationModel::getSerialisedScenario() const {
  return serialisedScenarioData;
}

std::string SimulationModel::getScenarioName() const { return scenarioName; }

std::string SimulationModel::getWorldName() const { return worldName; }
...
void SimulationModel::releaseManOverboard() {
  ...
  if (!manOverboard.getVisible()) {
    ...
    if (rand() > RAND_MAX / 2) {
      relativePosition.X = ownShip.getWidth() * 0.6 *
                           cos(ownShip.getHeading() * irr::core::DEGTORAD);
      relativePosition.Z = ownShip.getWidth() * -0.6 *
                           sin(ownShip.getHeading() * irr::core::DEGTORAD);
      // PositionEntity(mob,EntityX( ship_parent
      // )+(OwnShipWidth#*0.6)*Cos(angle#),THeight#,EntityZ( ship_parent
      // )-(OwnShipWidth#*0.6)*Sin(angle#), True)
    } else {
      relativePosition.X = ownShip.getWidth() * -0.6 *
                           cos(ownShip.getHeading() * irr::core::DEGTORAD);
      relativePosition.Z = ownShip.getWidth() * 0.6 *
...
void SimulationModel::retrieveManOverboard() { manOverboard.setVisible(false); }
...
void SimulationModel::setManOverboardPos(irr::f32 positionX,
                                         irr::f32 positionZ) {
  // To be used directly, eg when in secondary display mode only
  manOverboard.setPosition(irr::core::vector3df(positionX - offsetPosition.X, 0,
                                                positionZ - offsetPosition.Z));
}

bool SimulationModel::hasGPS() const { return ownShip.hasGPS(); }
...
void SimulationModel::startHorn() { sound->setVolumeHorn(1.0); }

void SimulationModel::endHorn() { sound->setVolumeHorn(0.0); }
...
void SimulationModel::update() {
  ...
  }
  {
    ...
    // Note that linear fog is hardcoded into the water shader, so should be
    // changed there if we use other fog types
    driver->setFog(light.getLightSColor(), irr::video::EFT_FOG_LINEAR,
                   0.01 * visibilityRange * M_IN_NM, visibilityRange * M_IN_NM,
                   0.00003f /*exp fog parameter*/, true, true);
  ...
  }
  {
    ...
    otherShips.update(
        deltaTime, scenarioTime, tideHeight, lightLevel, ownShip.getPosition(),
        ownShip.getLength());  // Update other ship motion (based on leg
                               // information), and light visibility.
  }
  {
    IPROF("Update buoys");
    // update buoys (for lights, floating, and if collision detection is turned
    // on)
    buoys.update(deltaTime, scenarioTime, tideHeight, lightLevel,
                 ownShip.getPosition(), ownShip.getLength());
  }
  {
  ...
  }
  {
    ...
    if (ownShip.getPosition().getLength() > 1000) {
      ...
      // Round to nearest 1000 metres - (multiple of water tile width, to avoid
      // jumps here)
      ...
      normalisedLogMessage.append(
          Utilities::lexical_cast<std::string>(offsetPosition.X));
      normalisedLogMessage.append(" Z: ");
      normalisedLogMessage.append(
          Utilities::lexical_cast<std::string>(offsetPosition.Z));
  ...
  if (radarCalculation.isRadarOn()) {
    {
      ...
      // set radar screen position, and update it with a radar image from the
      // radar calculation
      cursorPositionRadar = guiMain->getCursorPositionRadar();
    }
    {
      IPROF("Update radar calculation");
      // Choose which radar images to use, depending on the size of the display
      // being used
      if (2 * guiMain->getRadarPixelRadius() >
          radarImage->getDimension().Width) {
      ...
      radarCalculation.update(
          radarImageChosen, radarImageOverlaidChosen, offsetPosition, terrain,
          ownShip, buoys, otherShips, weather, rainIntensity, tideHeight,
          deltaTime, absoluteTime, cursorPositionRadar, isMouseDown);
    }
    {
  ...
  }
  {
    ...
    // calculate current angular elevation due to pitch and roll in the view
    // direction
    irr::f32 lookRadians = irr::core::degToRad(camera.getLook());
    elevAngle = -1 * ownShip.getPitch() * cos(lookRadians) +
                ownShip.getRoll() * sin(lookRadians) + camera.getLookUp();
  }
  {
    ...
    for (unsigned int i = 0; i < numberOfARPATracks; i++) {
      guiData->arpaContactStates.push_back(
          radarCalculation.getARPATrack(i).estimate);
    }
  }
  {
    ...
    guiData->rudder = ownShip.getRudder();  // inner workings of this will be
                                            // modified in model DEE
    guiData->wheel = ownShip.getWheel();    // inner workings of this will be
                                            // modified in model DEE
    ...
    guiData->enginePort =
        (ownShip.getPortEngine() * 270) -
        135;  // TODO: change gui control mapping so we don't need scaling here
    guiData->engineStbd =
        (ownShip.getStbdEngine() * 270) -
        135;  // TODO: change gui control mapping so we don't need scaling here
  ...
  }
  {
    ...
    // send data to gui
    guiMain->updateGuiData(
        guiData);  // Set GUI heading in degrees and speed (in m/s)
  }
}
...
bool SimulationModel::checkOwnShipCollision() {
  ...
  for (irr::u32 i = 0; i<numberOfOtherShips; i++) {
      ...
      irr::f32 bearingToOtherShipDeg = irr::core::radToDeg(atan2(relPosition.X,
  relPosition.Z));
      ...
      irr::f32 relativeBearingOtherShip = 180 + bearingToOtherShipDeg -
  otherShipHeading;

      //Find the minimum distance before a collision occurs
      irr::f32 minDistanceOwn =
  0.5*fabs(thisShipWidth*sin(irr::core::degToRad(relativeBearingOwnShip))) +
  0.5*fabs(thisShipLength*cos(irr::core::degToRad(relativeBearingOwnShip)));
      irr::f32 minDistanceOther =
  0.5*fabs(otherShipWidth*sin(irr::core::degToRad(relativeBearingOtherShip))) +
  0.5*fabs(otherShipLength*cos(irr::core::degToRad(relativeBearingOtherShip)));
  ...
  for (irr::u32 i = 0; i<numberOfBuoys; i++) { //Collision with buoy
      ...
      irr::f32 bearingToBuoyDeg = irr::core::radToDeg(atan2(relPosition.X,
  relPosition.Z));
