...
{
 public:
  SimulationModel(irr::IrrlichtDevice* dev, irr::scene::ISceneManager* scene,
                  GUIMain* gui, Sound* sound, ScenarioData scenarioData,
                  OperatingMode::Mode mode, irr::f32 viewAngle,
                  irr::f32 lookAngle, irr::f32 cameraMinDistance,
                  irr::f32 cameraMaxDistance, irr::u32 disableShaders,
  ...
  void setRateofTurn(
      irr::f32 rudder);  // Set the rate of turn (-ve is port, +ve is stbd)
  ...
  void setBallastTankPump(irr::f32 pump);
  void setWheel(
      irr::f32 wheel,
      bool force = false);  // Set the wheel (-ve is port, +ve is stbd) DEE. If
                            // force is true, the wheel change is applied even
                            // if the follow up rudder is failed
  ...
  void setStbdThrustLever(
      irr::f32);                  // sets starboard thrust lever range is 0..+1
  ...
  void btnIncrementPortSchottel();  // clockwise turn of the port schottel in
                                    // response to a key press
  void btnDecrementPortSchottel();  // anticlockwise turn of the port schottel
                                    // in response to a key press
  void btnIncrementStbdSchottel();  // clockwise turn of the starboard schottel
                                    // in response to a key press
  void btnDecrementStbdSchottel();  // anticlockwise turn of the starboard
                                    // schottel in response to a key press

  // DEE_NOV22 ^^^^

  void setPortEngine(
      irr::f32 port);  // Set the engine, (-ve astern, +ve ahead), range is +-1
  void setStbdEngine(
      irr::f32 stbd);  // Set the engine, (-ve astern, +ve ahead), range is +-1
  ...
  void setBowThrusterRate(
      irr::f32
          bowThrusterRate);  // Sets rate of change, for joystick button control
  void setSternThrusterRate(
      irr::f32 sternThrusterRate);  // Sets rate of change, for joystick button
                                    // control
  irr::f32 getBowThruster() const;
  irr::f32 getSternThruster() const;
  void setRudderPumpState(
      int whichPump,
      bool rudderPumpState);  // Sets how the rudder is responding. Assumed that
  ...
  irr::f32 getNoisyLat() const;
  irr::f32 getNoisyLong() const;
  ...
  irr::f32 getWaveHeight(irr::f32 posX, irr::f32 posZ)
      const;  // Return wave height (not tide) at the world position specified
  irr::core::vector2df getLocalNormals(irr::f32 relPosX,
                                       irr::f32 relPosZ) const;

  irr::core::vector2df getTidalStream(irr::f32 longitude, irr::f32 latitude,
                                      uint64_t absoluteTime)
      const;  // Tidal stream in m/s for the specified absolute position
  ...
  irr::f32 getTimeDelta() const;  // The change in time (s) since the start of
                                  // the start day of the scenario
  ...
  void changeOtherShipLeg(int shipNumber, int legNumber, irr::f32 bearing,
                          irr::f32 speed, irr::f32 distance);
  void addOtherShipLeg(int shipNumber, int afterLegNumber, irr::f32 bearing,
                       irr::f32 speed, irr::f32 distance);
  void deleteOtherShipLeg(int shipNumber, int legNumber);
  void resetOtherShipLegs(int shipNumber, irr::f32 course, irr::f32 speedKts,
                          irr::f32 distanceNm);
  ...
  void set_water_transparent(bool trans);
  void lookUp();
  void lookDown();
  void lookLeft();
  void lookRight();
  ...
  void setManOverboardVisible(bool visible);  // To be used directly, eg when in
                                              // secondary display mode only
  void setManOverboardPos(
      irr::f32 positionX,
      irr::f32 positionZ);  // To be used directly, eg when in secondary display
                            // mode only
  bool hasGPS() const;
 ...
 private:
  ...
  irr::video::IImage*
      radarImageLarge;  // Basic radar image, for full screen display
  irr::video::IImage*
      radarImageOverlaidLarge;  // WIth any 2d overlay, for full screen display
  irr::video::IImage*
      radarImageChosen;  // Should point to one of radarImage or radarImageLarge
  irr::video::IImage*
      radarImageOverlaidChosen;  // Should point to one of radarImageOverlaid or
                                 // radarImageOverlaidLarge
  // irr::f32 accelerator;
  irr::f32 tideHeight;
  irr::f32 weather;          // 0-12.0
  irr::f32 rainIntensity;    // 0-10
  irr::f32 visibilityRange;  // Nm
  irr::u32 loopNumber;  // u32 should be up to 4,294,967,295, so over 2 years at
                        // 60 fps
  irr::f32 zoom;
  Terrain terrain;
  Light light;
  ...
  irr::f32 deltaTime;
  irr::f32 scenarioTime;  // Simulation internal time, starting at zero at 0000h
                          // on start day of simulation
  uint64_t scenarioOffsetTime;  // Simulation day's start time from unix epoch
                                // (1 Jan 1970)
  uint64_t
      absoluteTime;  // Unix timestamp for current time, including start day.
                     // Calculated from scenarioTime and scenarioOffsetTime
