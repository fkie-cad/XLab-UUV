...
class MovingWaterSceneNode : public IMeshSceneNode,
 ...
 public:
  ...
  MovingWaterSceneNode(
      ISceneNode *parent, ISceneManager *mgr, ISceneNode *ownShip, s32 id,
      irr::u32 disableShaders, irr::u32 segments = 32,
  ...
  virtual IShadowVolumeSceneNode *addShadowVolumeSceneNode(
      const IMesh *shadowMesh = 0, s32 id = -1, bool zfailmethod = true,
 ...
 private:
  // Shader related
  int matWorldViewProjection;  // Identifiers, much faster than string
                               // matching...
  int matViewInverse;
  ...
  irr::video::IVideoDriver
      *driver;  // Here so we can save a call during the execution
