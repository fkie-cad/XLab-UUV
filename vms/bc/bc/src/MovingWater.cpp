...
MovingWaterSceneNode::MovingWaterSceneNode(
    ISceneNode *parent, ISceneManager *mgr, ISceneNode *ownShip, irr::s32 id,
    irr::u32 disableShaders, irr::u32 segments,
    ...
    : IMeshSceneNode(parent, mgr, id, position, rotation,
  ...
  // From Mel demo
  // (http://irrlicht.sourceforge.net/forum/viewtopic.php?f=9&t=51130&start=15#p296723)
  // START
  ...
  // cubemapConstants* cns = new
  // cubemapConstants(driverType==irr::video::EDT_OPENGL);
  // So far there are no materials ready to use a cubemap, so we provide our
  // own.
  irr::s32 water_shader = 0;
  irr::s32 transparent_water_shader = 0;
      ...
      water_shader =
          driver->getGPUProgrammingServices()
              ->addHighLevelShaderMaterialFromFiles(
                  "shaders/Water_vs.hlsl", "main", irr::video::EVST_VS_2_0,
                  "shaders/Water_ps.hlsl", "main", irr::video::EPST_PS_2_0,
                  // For callbacks
                  this, irr::video::EMT_SOLID);
      transparent_water_shader = water_shader;
    } else {
      // OpenGL
      water_shader =
          driver->getGPUProgrammingServices()
              ->addHighLevelShaderMaterialFromFiles(
                  "shaders/Water_vs.glsl", "main", irr::video::EVST_VS_2_0,
                  "shaders/Water_ps.glsl", "main", irr::video::EPST_PS_2_0,
                  // For callbacks
                  this, irr::video::EMT_SOLID);
      transparent_water_shader =
          driver->getGPUProgrammingServices()
              ->addHighLevelShaderMaterialFromFiles(
                  "shaders/Water_vs.glsl", "main", irr::video::EVST_VS_2_0,
                  "shaders/Water_transparent_ps.glsl", "main",
                  irr::video::EPST_PS_2_0,
                  // For callbacks
                  this, irr::video::EMT_TRANSPARENT_ALPHA_CHANNEL);
    }
  }
  // Just in case something goes horribly wrong...
  water_shader = water_shader == -1 ? 0 : water_shader;
  transparent_water_shader =
      transparent_water_shader == -1 ? 0 : transparent_water_shader;

  // FIXME: Hardcoded or defined in multiple places
  // Width in metres - Note this is used in Simulation model
  // normalisation as 100, so visible jumps in water are minimised
  tileWidth = 100;
  irr::f32 segmentSize = tileWidth / segments;

  // Note that the A and w parameters will get overwritten by
  // ocean->resetParameters() dependent on the model's weather
  ocean = new cOcean(segments, 0.00005f, vector2(32.0f, 32.0f), tileWidth);

  mesh = mgr->addHillPlaneMesh(
      "myHill", irr::core::dimension2d<irr::f32>(segmentSize, segmentSize),
      irr::core::dimension2d<irr::u32>(segments, segments), 0, 0.0f,
      irr::core::dimension2d<irr::f32>(0, 0),
      irr::core::dimension2d<irr::f32>(tileWidth / (irr::f32)(segments),
                                       tileWidth / (irr::f32)(segments)));
  transparent_mesh = mgr->addHillPlaneMesh(
      "myTransparentHill",
  ...
  if (!flatMesh) {
    std::cerr << "Could not load flat sea mesh from media/flatsea.x"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  */
  ...
  if (!disableShaders) {
    _camera = mgr->addCameraSceneNode(0, irr::core::vector3df(0, 0, 0),
                                      irr::core::vector3df(0, 0, 0), -1, false);
    irr::video::ITexture *bumpTexture =
        driver->getTexture("/media/waterbump.png");

    // _refractionMap = _videoDriver->addRenderTargetTexture(renderTargetSize);
    // TODO: Check hardcoding here
    _reflectionMap = driver->addRenderTargetTexture(
        irr::core::dimension2d<irr::u32>(512, 512));
    ...
    for (irr::u32 i = 0; i < mesh->getMeshBufferCount(); ++i) {
      ...
      if (mb) {
        ...
        mb->getMaterial().MaterialType =
            (irr::video::E_MATERIAL_TYPE)water_shader;
        mb->getMaterial().FogEnable = true;
      }
    }

    for (irr::u32 i = 0; i < transparent_mesh->getMeshBufferCount(); ++i) {
      scene::IMeshBuffer *mb = transparent_mesh->getMeshBuffer(i);
      if (mb) {
        mb->getMaterial().setTexture(0, bumpTexture);
        mb->getMaterial().setTexture(1, _reflectionMap);
        mb->getMaterial().MaterialType =
            (irr::video::E_MATERIAL_TYPE)transparent_water_shader;
        mb->getMaterial().FogEnable = true;
      }
    }
  ...
  // Hard code bounding box to be large - we always want to render water, and we
  // actually render multiple displaced copies of the mesh, so just getting the
  // mesh bounding box isn't correct.
  // TODO: Look here if there's a problem with the water disappearing or if we
  // implement collision with water.
  boundingBox =
      irr::core::aabbox3d<irr::f32>(-10000, -100, -10000, 10000, 100, 10000);
}

//! destructor
MovingWaterSceneNode::~MovingWaterSceneNode() {
  // Mesh is dropped in IMeshSceneNode destructor (??? FIXME: Probably not
  // true!)
  delete ocean;
...
void MovingWaterSceneNode::OnSetConstants(
    video::IMaterialRendererServices *services, irr::s32 userData) {
  ...
  if (!disableShaders) {
    if (firstRun) {
      ...
      matWorldReflectionViewProj =
          services->getVertexShaderConstantID("WorldReflectionViewProj");
      ...
      if (IsOpenGL) {
        ...
        matWorldViewProjection =
            services->getVertexShaderConstantID("matWorldViewProjection");
    ...
    irr::core::matrix4 worldReflectionViewProj =
    ...
    services->setVertexShaderConstant(matWorldReflectionViewProj,
                                      worldReflectionViewProj.pointer(), 16);
    ...
    if (IsOpenGL) {
      ...
      services->setVertexShaderConstant(matWorldViewProjection, mat.pointer(),
                                        16);
...
void MovingWaterSceneNode::OnRegisterSceneNode() {
  ...
  if (IsVisible) {
    ...
    // Additionally register as a transparent effect
...
void MovingWaterSceneNode::OnAnimate(irr::u32 timeMs) {
  ...
  if (transparent) current_mesh = transparent_mesh;
  if (current_mesh && IsVisible) {
    // Set light level
    video::SColorf ambientLight = this->getSceneManager()->getAmbientLight();
    // Average
    lightLevel = (ambientLight.r + ambientLight.g + ambientLight.b) / 3.0;
    ...
    const irr::u32 meshBufferCount = current_mesh->getMeshBufferCount();

    for (irr::u32 b = 0; b < meshBufferCount; ++b) {
      const irr::u32 vtxCnt = current_mesh->getMeshBuffer(b)->getVertexCount();

      for (irr::u32 i = 0; i < vtxCnt; ++i) {
        current_mesh->getMeshBuffer(b)->getPosition(i).X =
            -1 *
            vertices[i].x;  // Swap sign to maintain correct rotation order of
                            // vertices: TODO: Look at basic definition of X and
                            // Z coordinate system between water and FFTWave
        current_mesh->getMeshBuffer(b)->getPosition(i).Y = vertices[i].y;
        current_mesh->getMeshBuffer(b)->getPosition(i).Z = vertices[i].z;

        // Set normals (TODO: Disable normal calculation in FFT for speed)
        // current_mesh->getMeshBuffer(b)->getNormal(i).X = -1*vertices[i].nx;
        // current_mesh->getMeshBuffer(b)->getNormal(i).Y = vertices[i].ny;
        // current_mesh->getMeshBuffer(b)->getNormal(i).Z = vertices[i].nz;
      }
      // Manually recalculate normals
      SceneManager->getMeshManipulator()->recalculateNormals(
          current_mesh->getMeshBuffer(b));
    }  // end for all mesh buffers
    current_mesh->setDirty(scene::EBT_VERTEX);
  }

  IMeshSceneNode::OnAnimate(timeMs);
  // Fixme: Need to store timeMs in something accessible to the shader for
  // ripples
  ...
  if (IsVisible && !disableShaders) {
    ...
    irr::core::rect<irr::s32> currentViewPort =
        driver->getViewPort();  // Get the previous viewPort
    ...
    driver->setRenderTarget(
        _reflectionMap,
        irr::video::ECBF_COLOR |
            irr::video::ECBF_DEPTH);  // render to reflection
    ...
    irr::f32 renderScale = 1.5;  // This matches the scaling in the shader, to
                                 // avoid artefacts near the edge of the screen
    irr::f32 renderFOV =
        2 * atan(renderScale * tan(currentCamera->getFOV() / 2));
    _camera->setFOV(renderFOV);

    irr::core::vector3df position = currentCamera->getAbsolutePosition();
    position.Y =
        -position.Y + 2 * RelativeTranslation.Y;  // position of the water
    ...
    irr::core::plane3d<irr::f32> reflectionClipPlane(
        0, RelativeTranslation.Y - CLIP_PLANE_OFFSET_Y, 0, 0, 1, 0);
    ...
    driver->setViewPort(irr::core::rect<irr::s32>(
        0, 0, 10, 10));  // Set to a dummy value first to force the next call to
                         // make the change
...
irr::f32 MovingWaterSceneNode::getWaveHeight(irr::f32 relPosX,
                                             irr::f32 relPosZ) const {
  // Adjust relative position by 1/2 tile width

  // Get the wave height (not including tide height) at this position relative
  ...
  while (relPosXInternal < 0) relPosXInternal += tileWidth;
  while (relPosZInternal < 0) relPosZInternal += tileWidth;
  ...
  xIndexFloat = (segments + 1) -
                xIndexFloat;  // Sign of x is flipped when heights are applied!

  // std::cout << "xIndexF:" << xIndexFloat << " zIndexF:" << zIndexFloat << "
  // segments+1:" << segments+1 << std::endl;
  ...
  if (xIndex0 == (segments + 1)) {
    xIndex0 = 0;
  }
  if (zIndex0 == (segments + 1)) {
    zIndex0 = 0;
  }
  if (xIndex1 == (segments + 1)) {
    xIndex1 = 0;
  }
  if (zIndex1 == (segments + 1)) {
    zIndex1 = 0;
  }
  ...
  irr::f32 localHeight = height00 * (1 - interpX) * (1 - interpZ) +
                         height10 * interpX * (1 - interpZ) +
                         height01 * (1 - interpX) * interpZ +
                         height11 * interpX * interpZ;
...
irr::core::vector2df MovingWaterSceneNode::getLocalNormals(
    irr::f32 relPosX, irr::f32 relPosZ) const {
  // Adjust relative position by 1/2 tile width
  ...
  irr::f32 localNx = nx00 * (1 - interpX) * (1 - interpZ) +
                     nx10 * interpX * (1 - interpZ) +
                     nx01 * (1 - interpX) * interpZ + nx11 * interpX * interpZ;
  irr::f32 localNz = nz00 * (1 - interpX) * (1 - interpZ) +
                     nz10 * interpX * (1 - interpZ) +
                     nz01 * (1 - interpX) * interpZ + nz11 * interpX * interpZ;

  if (localisnan(localNx) || localisinf(localNx) || localisnan(localNz) ||
      localisinf(localNz)) {
...
void MovingWaterSceneNode::set_transparent(bool trans) {
...
void MovingWaterSceneNode::render() {
  ...
  // which render pass are we on?
  bool is_transparent_pass =
      SceneManager->getSceneNodeRenderPass() == scene::ESNRP_TRANSPARENT_EFFECT;

  // Draw main water
  if (!is_transparent_pass) {
    if (!this->transparent) {
      // if we're not requesting transparent water, draw nearby water using
      // the opaque shader
      for (irr::u32 i = 0; i < mesh->getMeshBufferCount(); ++i) {
        scene::IMeshBuffer *mb = mesh->getMeshBuffer(i);
        if (mb) {
          ...
          irr::core::vector3df basicPosition =
              AbsoluteTransformation.getTranslation();

          // Draw multiple copies of tileable water
          for (int j = -10; j <= 10; j++) {
            for (int k = -10; k <= 10; k++) {
              AbsoluteTransformation.setTranslation(
                  basicPosition +
                  irr::core::vector3df(j * tileWidth, 0, k * tileWidth));
              driver->setTransform(video::ETS_WORLD, AbsoluteTransformation);
    ...
    // Always draw it using the opaque shader, we only care about making
    // the water transparent around Ownship
    for (irr::u32 i = 0; i < flatMesh->getMeshBufferCount(); ++i) {
      scene::IMeshBuffer *mb = flatMesh->getMeshBuffer(i);
      if (mb) {
        const video::SMaterial &material = mb->getMaterial();
    ...
    // transparent pass, do we want transparent water? now's the right pass
    if (this->transparent) {
      for (irr::u32 i = 0; i < transparent_mesh->getMeshBufferCount(); ++i) {
        scene::IMeshBuffer *mb = transparent_mesh->getMeshBuffer(i);
        if (mb) {
...
const irr::core::aabbox3d<irr::f32> &MovingWaterSceneNode::getBoundingBox()
    const {
  return boundingBox;
}
...
IShadowVolumeSceneNode *MovingWaterSceneNode::addShadowVolumeSceneNode(
    const IMesh *shadowMesh, irr::s32 id, bool zfailmethod, irr::f32 infinity) {
...
void MovingWaterSceneNode::setMaterialTexture(irr::u32 textureLayer,
